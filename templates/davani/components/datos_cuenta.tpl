...
<div role="tabpanel">
  ...
  <div class="tab-content">
    <div role="tabpanel" class="tab-pane active" id="cuenta">
            <form id="form-mod" action="javascript:enviarDatosAjax('{$URL_ACTUAL}/guardar',$('#form-mod').serialize(),'POST',false,false)">
                <div class="col-xs-12 col-sm-6">
                    <label for="">Nombre usuario<b class="text-danger">*</b></label>
                    <input type="text" class="form-control" name="nombre" value="{$DATOS_USUARIO.nombre}" required="true"  />
                    <label for="">Nombre completo<b class="text-danger">*</b></label>
                    <input type="text" class="form-control" name="nombre_completo" value="{$DATOS_USUARIO.nombre_completo}" required="true" />
                    <label for="">Email de la cuenta<b class="text-danger">*</b></label>
                    <input type="text" class="form-control" name="email" value="{$DATOS_USUARIO.email}" required="true" />
                    ...
                    <input type="hidden" class="" name="id" value="{$DATOS_USUARIO.id}" />
                </div>
                <div class="col-xs-12 col-sm-6">
                    <b>Fecha de creación de la cuenta</b><br>
                    {$DATOS_USUARIO.registerDate}
                </div>
      ...
      <div role="tabpanel" class="tab-pane" id="facturacion">
          <form id="datos_facturacion" action="javascript:enviarDatosAjax('{$URL_ACTUAL}/guardar-fact',$('#datos_facturacion').serialize(),'POST',false,false)">

              <input type="hidden" class="" name="usuario_id" value="{$DATOS_USUARIO.id}" />
                 <div class="col-xs-12 col-sm-2">
                  <label for="">Trato <b class="text-danger">*</b></label>
                  <select name="title" class="form-control" >
                     {if $DATOS_FACTURACION.title =='Sr'}
              ...
              <div class="col-xs-12 col-sm-6">
                  ...
                  <input type="text" class="form-control" name="nombre" value="{$DATOS_FACTURACION.name}" required="true" />

                  <label for="">Apellidos <b class="text-danger">*</b></label>
                  <input type="text" class="form-control" name="apellidos" value="{$DATOS_FACTURACION.middle_name}" required="true" />
                  <label for="">Companìa <b class="text-danger">*</b></label>
                  <input type="text" class="form-control" name="company" value="{$DATOS_FACTURACION.company}" required="true" />
                  <label for="">CIF/DNI/NIE <b class="text-danger">*</b></label>
                  <input type="text" class="form-control" name="cif" value="{$DATOS_FACTURACION.NIFDNINIE}" required="true" />
                  <label for="">Paìs <b class="text-danger">*</b></label>
                  <select name="pais" class="form-control" id="pais_f" onchange="enviarDatosAjax('{$URL_ACTUAL}/get_provincias',{literal}{{/literal}id:$(this).val(){literal}}{/literal},'POST','#select_provincias_f',false)" required="true" >
                        <option value="">Selecciona un paìs</option>

                {foreach item=item from=$LISTA_PAISES}
                    {if $DATOS_FACTURACION.virtuemart_country_id == $item.id }
                ...
                {/foreach}
                  ...
                  <!--  <label for="">Ciudad </label>
                    ...
                    <input type="text" class="form-control" name="ciudad" value="{$DATOS_FACTURACION.city}" required="true" />
                  <label for="">CP  <b class="text-danger">*</b> </label>
                  <input type="text" class="form-control" name="cp" value="{$DATOS_FACTURACION.zip}" required="true" />
                  <label for="">Direcciòn de facturación  <b class="text-danger">*</b></label>
                  <input type="text" class="form-control" name="direccion" value="{$DATOS_FACTURACION.address_1}" required="true" />
                  <label for="">Telèfono  <b class="text-danger">*</b></label>
                  <input type="text" class="form-control" name="telefono" value="{$DATOS_FACTURACION.phone_1}" required="true" />
                  <label for="">Fax</label>
                  <input type="text" class="form-control" name="fax" value="{$DATOS_FACTURACION.fax}"  />
                  <p class="text-center ">
    ...
    <div role="tabpanel" class="tab-pane" id="envio">
         <div id="form_dir_env" class="col-xs-12 col-sm-12">
                 ...
                 <div class="col-xs-12 col-sm-12">
                         ...
                         {foreach item=item from=$DATOS_ENVIO}
                             <div  class="col-xs-12 col-sm-6">
                                 <div class="offer offer-info">
                                     <div class="shape" >
                                         <div  class="shape-text" onclick="javascript:enviarDatosAjax('{$URL_ACTUAL}/delete-dir',{literal}{{/literal}id:{$item.id}{literal}}{/literal},'POST','#form_dir_env',false)">
                                             X
                                         </div>
                                     </div>
                                     <div class="offer-content" onclick="javascript:enviarDatosAjax('{$URL_ACTUAL}/update-dir',{literal}{{/literal}id:{$item.id}{literal}}{/literal},'POST','#form_dir_env',false)">
                                         ...
                                         <p>
                                             {$item.city}, {$item.provincia}, {$item.pais}
                                         </p>
                                     </div>
                                 </div>
                             </div>
          ...
          <div class="col-xs-12 col-sm-6" id="insert_div">
              <button class="btn btn-warning"
                      onclick="javascript:enviarDatosAjax('{$URL_ACTUAL}/insert-dir',{literal}{{/literal}{literal}}{/literal},'POST','#form_dir_env',false)" >Nueva direccion de envio
              </button>
           </div>
         </div>
    </div>
